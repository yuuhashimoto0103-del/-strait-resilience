...
def merge_and_translate(fresh: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for a in fresh:
        ...
        if existing and existing.get("translation_status") == "complete":
...
def build_weekly(client, articles: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if client is None:
        return {
            ...
            "ja": {"headline":"Weekly Briefは自動生成待ちです","summary":"OPENAI_API_KEYをGitHub ActionsのSecretに設定すると、1週間の主要Signalが日本語・韓国語で自動生成されます。","points":[]},
            "ko": {"headline":"Weekly Brief 자동 생성을 준비 중입니다","summary":"GitHub Actions Secret에 OPENAI_API_KEY를 설정하면 일주일의 핵심 Signal을 일본어·한국어로 자동 생성합니다.","points":[]}
        }
